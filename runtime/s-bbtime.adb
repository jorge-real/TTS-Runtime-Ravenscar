...
with System.Tasking; -- Added to check if current task is inside a PO
...
package body System.BB.Time is
   ...
   use System.Tasking; -- Added to check if current task is inside a PO
   ...
   procedure Alarm_Handler (Interrupt : Interrupts.Interrupt_ID) is
      ...
      T_Id            : constant Task_Id := To_Task_Id (Thread_Self.ATCB);
   begin
      ...
      then
         ...
         Thread_Self.Timing_Events_Triggered := Now;
         Thread_Self.Timing_Events_Pending := True;
      end if;
